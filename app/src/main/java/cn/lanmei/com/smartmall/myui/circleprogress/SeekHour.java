...
public class SeekHour extends View {
    ...
    public SeekHour(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public SeekHour(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        radius_2 = mRadius-mTextWidth*2.8f;
        if (mProgress>=count)
            cirAngle=rotate*(mProgress-count-1)-90+rotate;
        if (inRangeR_1){
            xChartCalc.CalcArcEndPointXY(0f,0f,radius_1,cirAngle);
        }else{
            xChartCalc.CalcArcEndPointXY(0f,0f,radius_2,cirAngle);
        }
    ...
    protected boolean updateProgress(int progress)
    {
        ...
        if (progress != mProgress||progress==0||progress==maxProgress/2)
        {
            mProgress = progress;
            invalidate();
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        boolean inRangeR_2 = Math.abs(distance - radius_2) <= mSectionHeight;
        boolean inRange = inRangeR_1||inRangeR_2;
...
//				SystemConst.seekCiecleFirst = false;
        ...
        if (updateProgress)
        {
            ...
            int progress = Math.round(position * ((float) maxProgress/2));
...
//            L.MyLog("","inRangeR_1:"+inRangeR_1+"---inRangeR_2:"+inRangeR_2+"---progress:"+progress);
            if (inRangeR_1){
                progress=maxProgress/2+progress;
                if (progress==12)
                    progress=0;
            }else{
                if (progress==0)
