...
/**商家维修商*/
public class Activity_list_shop extends BaseActionActivity {
    ...
    public void loadViewLayout() {
        boolean isFixShop=false;
        if (getIntent()!=null)
            isFixShop=getIntent().getBooleanExtra("isFixShop",false);

        setMContentView(F_nearby_merchant.newInstance(isFixShop) );
    }
    ...
    private void getPersimmions() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (permissions.size() > 0) {
                requestPermissions(permissions.toArray(new String[permissions.size()]), Activity_complete_info.SDK_PERMISSION_REQUEST);
            }
        }
    }
