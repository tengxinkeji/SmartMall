...
public class __EsptouchTaskAll implements __IEsptouchTask {
...
//	private volatile boolean mIsSuc = false;
	private volatile boolean mIsReceiveStop = false;
	...
	public __EsptouchTaskAll(String apSsid, String apBssid, String apPassword, Context context, IEsptouchTaskParameter parameter
...
//				while (correctBroadcastCount < mParameter.getThresholdSucBroadcastCount()) {
				mIsReceiveStop=receiveTimestamp >= mParameter.getmIntervalScanWifiisecond();
				while (!mIsReceiveStop) {
					mIsReceiveStop=receiveTimestamp >= mParameter.getmIntervalScanWifiisecond();
					Log.i(TAG, "receive  receiveTimestamp："+receiveTimestamp);
					receiveBytes = mSocketServer
							.receiveSpecLenBytes(expectDataLen);
					if (receiveBytes==null){
						if (__IEsptouchTask.DEBUG) {

							Log.i(TAG, "receiveBytes:"+receiveBytes);
						}
						mEsptouchResult = new EsptouchResult(false, null,
								null);
						mIsReceiveStop=true;
						break;
					}
					...
					if (receiveOneByte == expectOneByte) {
						...
						if (!MyApplication.wifiBssidcontainsKey(mBssid)){
							receiveTimestamp=System.currentTimeMillis()-countTimestamp;
						...
						mEsptouchResult = new EsptouchResult(true,
								mBssid, inetAddress);
						MyApplication.putWifiDevScan(mBssid,mEsptouchResult);
						MyApplication.setCurrentWifiKey(mBssid);
						if (__IEsptouchTask.DEBUG) {
							Log.i(TAG, "receive " + correctBroadcastCount
									+ " correct broadcast----"+receiveTimestamp);
						}
						...
						if (timeout < 0) {
							...
							&& receiveBytes == null) {
						if (__IEsptouchTask.DEBUG) {
							Log.i(TAG, "esptouch timeout 3---expectDataLen:"+expectDataLen);
						}
						mEsptouchResult = new EsptouchResult(false, null,
								null);
						mIsReceiveStop=true;
						...
						countTimestamp=System.currentTimeMillis();
						if (__IEsptouchTask.DEBUG) {
							Log.i(TAG, "receive rubbish message, just ignore----"+receiveTimestamp);
						}
					}

				}
				__EsptouchTaskAll.this.__interrupt();
				if (__IEsptouchTask.DEBUG) {
...
//			index = (index + ONE_DATA_LEN) % dcBytes2.length;
	...
	public IEsptouchResult executeForResult() throws RuntimeException {
		...
		MyApplication.wifiDevScanClean();
		...
		try {
			...
			if (MyApplication.getScanWifiCount()>0)
			{
				return mEsptouchResult;
			}
			else
