...
public class DemoDBManager {
    ...
    synchronized public void saveContactList(List<EaseUser> contactList) {
        ...
        if (db.isOpen()) {
            ...
            for (EaseUser user : contactList) {
                ...
                if (uid.startsWith("w_")){
                    uids+=uid+",";
                }else {
                    ContentValues values = new ContentValues();
                    values.put(UserDao.COLUMN_NAME_ID, uid);
    ...
    public  void updateNickImg(final String uids){
        new AsyncTask<SQLiteDatabase,Void,Void>(){
            ...
            protected Void doInBackground(SQLiteDatabase... params) {
                ...
                requestParams.addParam("mid",uids);
                requestParams.setBaseParser(new ParserJson());
                JSONObject obj = (JSONObject) URLRequest.requestByGet(requestParams);
                try {
                       ...
                       JSONArray data=obj.optJSONArray("data");
                        if (data!=null&&data.length()>0){
                            for(int i=0;i<data.length();i++){
                                ...
                                ContentValues values = new ContentValues();
                                values.put(UserDao.COLUMN_NAME_ID, item.getInt("id"));
                                String nickname=item.getString("nickname");
                                if(nickname != null||nickname.equals("null"))
                                    values.put(UserDao.COLUMN_NAME_NICK, nickname);
                                String pic=item.getString("pic");
                                if(pic != null||pic.equals("null"))
                                    values.put(UserDao.COLUMN_NAME_AVATAR, pic);
                                db.replace(UserDao.TABLE_NAME, null, values);
                            }
                        }
                    }
            ...
            @Override
            protected void onPostExecute(Void contentValues) {
